...
public class PointOctreeNode<T> where T : class {
	...
	public IList<T> GetNearby(Ray ray, float maxDistance) {
		...
		if (children != null) {
			for (int i = 0; i < 8; i++) {
				var childColliding = children[i].GetNearby(ray, maxDistance);
    ...
    /// Author: Leonardo Scur
    /// <summary>
    /// Returns objects that are within <param name="maxDistance">maxDistance</param> of the specified point.
    /// </summary>
    /// <param name="point">The position to look nearby</param>
    /// <param name="maxDistance">Maximum distance to consider.</param>
    /// <returns>Objects within range.</returns>
    public IList<T>  GetNearby(Vector3 point, float maxDistance) {
        var collidingWith = new List<T>();

        for (int i = objects.Count - 1; i >= 0; i--) {
            if (Vector3.Distance(point, objects[i].Pos) <= maxDistance) {
                collidingWith.Add(objects[i].Obj);
            }
        }
        ...
        if (children != null) {
            for (var i = 0; i < 8; i++) {
                var childColliding = children[i].GetNearby(point, maxDistance);
                if (childColliding != null) collidingWith.AddRange(childColliding);
            }
        }
